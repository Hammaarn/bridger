...
|---|---|---|
| ~~1~~ | ~~**LIGHT MODE: commit, design, or toggle**~~ | **DECIDED S#285 — Erik took A, pin dark.** The light palette is removed, not hidden: the `@media (prefers-color-scheme: light)` block is gone from `globals.css` and the served stylesheet contains zero occurrences of `prefers-color-scheme`, so there is no rule left to trigger. `themeColor` is a single value. I had recommended C (dark + toggle) and A is the better call: a toggle still owes the second design, and the second design was the cost. See `DECISIONS.md` 2026-08-31. |
